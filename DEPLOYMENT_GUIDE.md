...
5. أضف متغيرات بيئة وقت التشغيل (Runtime) في Vercel → Settings → Environment Variables:
   ...
   - `DIRECT_LOGIN_PASSWORD` (اختياري — دخول المالك المباشر بديل عند غياب بوابة OAuth).
     عند ضبطه تُفعَّل صفحة `/owner-login` ونقطة `POST /api/auth/direct-login`،
     ويُسجَّل حساب المالك دائماً بدور **SUPER_ADMIN**. اتركه فارغاً لتعطيله (404).
     **بدون أي ضبط مسبق:** افتح `/owner-login` مرة واحدة لضبط كلمة مرور المالك،
...
2. **`DIRECT_LOGIN_PASSWORD` يُسجّل تحذيراً في الإنتاج:** عند ضبطه في بيئة الإنتاج يُسجَّل تحذير أمني `SECURITY: direct-login backdoor enabled in production` ثم يستمر الإقلاع. يُنصح بشدة بأن يكون **فارغاً** في الإنتاج لتعطيل `POST /api/auth/direct-login` و `/owner-login` (الاستجابة 404).
