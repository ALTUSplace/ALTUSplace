...
export const DigitalContractModal: React.FC<ContractProps> = ({
  bookingId = 'ALT-' + Math.floor(100000 + Math.random() * 900000),
  customerName,
  vendorName,
  itemName,
  itemType,
  ...
  const handleSign = (e: React.FormEvent) => {
    ...
    setIsSigned(true);
    toast.success('تم توقيع العقد الرقمي بنجاح وتوثيقه عبر الوسيط المعتمد ALTUSplace');
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto bg-slate-900 dark:bg-[#1C1C1E] text-slate-100 dark:text-[#F1F1F3] border-slate-800 dark:border-[#2C2C2E]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl font-bold text-amber-400">
            <ShieldCheck className="w-6 h-6 text-amber-500" />
            عقد وساطة رقمي معتمد - منصة ALTUSplace
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4 print:bg-white print:text-black">
          ...
          {/* Parties */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-950 dark:bg-[#111113] p-4 rounded-xl border border-slate-800 dark:border-[#2C2C2E] print:bg-slate-100">
            <div>
          ...
          {/* Signature Section */}
          <div className="border-t border-slate-800 dark:border-[#2C2C2E] pt-4">
            {isSigned ? (
              <div className="bg-cyan-500/10 border border-cyan-500/30 p-4 rounded-xl flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-300 dark:text-[#D6D6DB]">الموقع: {signatureName} | التوقيت: {new Date().toLocaleString('ar-MA')}</p>
                  </div>
                </div>
                <Button onClick={handlePrint} variant="outline" size="sm" className="gap-2 bg-slate-800 dark:bg-[#2C2C2E] text-slate-200 dark:text-[#E8E8EB] border-slate-700 dark:border-[#48484D]">
                  <Printer className="w-4 h-4" />
                  طباعة / تحميل PDF
                </Button>
              </div>
            ) : (
              <form onSubmit={handleSign} className="bg-slate-950 dark:bg-[#111113] p-4 rounded-xl border border-slate-800 dark:border-[#2C2C2E] space-y-3">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-slate-900 dark:bg-[#1C1C1E] border border-slate-700 dark:border-[#48484D] rounded-lg px-3 py-2 text-sm text-slate-100 dark:text-[#F1F1F3] focus:outline-none focus:border-amber-500"
                  />
