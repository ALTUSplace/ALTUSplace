...
export default function HostDashboard() {
  ...
  const resubmitRejected = trpc.listings.resubmitRejected.useMutation({ onSuccess: async () => { await Promise.all([listings.refetch(), statusSummary.refetch(), overview.refetch()]); setEditingId(null); setResubmissionId(null); setShowForm(false); toast.success("تمت إعادة إرسال الإعلان للمراجعة."); }, onError: (error) => toast.error(error.message) });
  const toggleAvailability = trpc.listings.toggleAvailability.useMutation({ onSuccess: async () => { await Promise.all([listings.refetch(), overview.refetch()]); toast.success("تم تحديث حالة التوفر."); }, onError: (error) => toast.error(error.message) });
  ...
  const submit = (event: React.FormEvent) => {
    ...
    if (resubmissionId) {
      if (!form.imageUrl || !form.imageVerificationProof) return toast.error("يرجى رفع صورة أصلية جديدة واجتياز الفحص قبل إعادة الإرسال");
      resubmitRejected.mutate({ id: resubmissionId, title: form.title, city: form.city, pricePerDay: Number(form.price), description: form.description, officeType: form.officeType, rentalPeriod: form.rentalPeriod as "daily" | "monthly" | "yearly", amenities: form.amenities, imageUrl: form.imageUrl, imageVerificationProof: form.imageVerificationProof });
      return;
    }
    if (editingId) {
      updateListing.mutate({ id: editingId, title: form.title, city: form.city, pricePerDay: Number(form.price), description: form.description, officeType: form.officeType, rentalPeriod: form.rentalPeriod as "daily" | "monthly" | "yearly", amenities: form.amenities, ...(form.imageUrl ? { imageUrl: form.imageUrl, imageVerificationProof: form.imageVerificationProof } : {}) });
      return;
    }
    if (!form.imageUrl || !form.imageVerificationProof) return toast.error("يرجى رفع صورة أصلية واجتياز الفحص قبل نشر المكتب");
    createListing.mutate({ title: form.title, city: form.city, category: "office", pricePerDay: Number(form.price), description: form.description, officeType: form.officeType, rentalPeriod: form.rentalPeriod as "daily" | "monthly" | "yearly", amenities: form.amenities, imageUrl: form.imageUrl, imageVerificationProof: form.imageVerificationProof });
  };
