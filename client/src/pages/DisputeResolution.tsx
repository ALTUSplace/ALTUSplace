        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          ...
          <Dialog open={isOpenModal} onOpenChange={setIsOpenModal}>
            <DialogTrigger asChild>
              <Button className="bg-[#1C1C1E] hover:bg-[#1C1C1E]/90 text-white gap-2 font-medium shadow-md"><PlusCircle className="w-4 h-4" /> فتح نزاع جديد وإرفاق أدلة</Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleCreateDispute} className="space-y-4 pt-3">
                ...
                <DialogFooter className="pt-2"><Button type="submit" disabled={createDispute.isPending} className="w-full bg-[#1C1C1E] hover:bg-[#1C1C1E]/90 disabled:opacity-60 text-white font-medium">{createDispute.isPending ? 'جاري إرسال الطلب والأدلة...' : 'إرسال الشكوى والأدلة وتفعيل لجنة الوساطة'}</Button></DialogFooter>
              </form>
            </DialogContent>
          </Dialog>
