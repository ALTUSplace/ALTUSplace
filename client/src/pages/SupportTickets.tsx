  ...
  return (
    <div className="min-h-screen bg-background text-foreground py-8 px-4" dir={direction}>
      <div className="container mx-auto max-w-5xl space-y-8">
        <div className="bg-[#15120D] border border-slate-800 p-6 sm:p-8 rounded-3xl shadow-xl flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        {isCreating && (
          <form onSubmit={handleCreateTicket} className="bg-[#15120D] border border-slate-800 p-6 sm:p-8 rounded-3xl shadow-xl space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <label className="space-y-2">
                ...
                <input
                  ...
                  className="w-full bg-[#15120D] border border-slate-700 rounded-xl px-4 py-3 text-xs text-white focus:outline-none focus:border-amber-500"
                  maxLength={255}
                  required
                />
              </label>
            ...
            <label className="space-y-2 block">
              ...
              <textarea
                ...
                className="w-full bg-[#15120D] border border-slate-700 rounded-xl p-4 text-xs text-white focus:outline-none focus:border-amber-500"
                maxLength={5000}
                required
              />
            </label>
            <div className="flex justify-end gap-3">
              <Button type="button" variant="outline" onClick={() => setIsCreating(false)} className="border-slate-700 text-slate-300 hover:bg-[#15120D] text-xs px-6 py-2.5 rounded-xl">
                إلغاء
              </Button>
        ...
        <div className="bg-[#15120D] border border-slate-800 p-6 sm:p-8 rounded-3xl shadow-xl space-y-6">
          ...
          {ticketsQuery.isLoading ? (
            <div className="rounded-2xl border border-slate-800 bg-[#15120D] p-10 text-center text-slate-400">جاري تحميل تذاكرك...</div>
            ...
            <div className="rounded-2xl border border-dashed border-slate-700 bg-[#15120D]/60 p-10 text-center text-slate-400">لا توجد تذاكر مرتبطة بحسابك. افتح تذكرة جديدة عند الحاجة.</div>
          ) : (
            <div className="space-y-4">
              {tickets.map((ticket) => (
                <article key={ticket.id} className="bg-[#15120D] border border-slate-800 p-5 sm:p-6 rounded-2xl space-y-3 shadow-lg">
                  ...
                  <div className="text-xs text-slate-300 bg-[#15120D]/60 p-4 rounded-xl border border-slate-800/80 flex items-start gap-3">
